...
/*   Updated: 2019/10/06 19:22:13 by aorji            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
public:
    explicit Puzzle(int size): _puzzle_size(size), _puzzle_state(NONE) { set_goal_state(); }
    ...
    void    set_goal_state(void) { 
        for(int i = 1; i < _puzzle_size * _puzzle_size; ++i)
            _goal_state.push_back(new Point(0, 0, i));
        _goal_state.push_back(new Point(0, 0, 0));
    }
    ...
    void    fill_puzzle(void)
    {
        for(int i = 0; i < _puzzle_size; ++i)
        {
            for(int j = 0; j < _puzzle_size; ++j)
            {
                int value = 0;
                std::cin >> value;
                _data.push_back(new Point(i, j, value));
                if (value == 0)
                {
                    static char times = 0;
                    ++times;
